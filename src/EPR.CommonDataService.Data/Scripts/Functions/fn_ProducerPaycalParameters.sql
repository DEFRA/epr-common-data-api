IF EXISTS (SELECT 1 FROM sys.sql_modules WHERE object_id = OBJECT_ID(N'[dbo].[fn_ProducerPaycalParameters]'))
    DROP FUNCTION [dbo].[fn_ProducerPaycalParameters];
GO

CREATE FUNCTION [dbo].[fn_ProducerPaycalParameters] (@FileId [UNIQUEIDENTIFIER]) RETURNS TABLE
AS
RETURN ( 
    WITH 
