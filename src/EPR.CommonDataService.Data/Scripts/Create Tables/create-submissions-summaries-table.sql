...
FROM sys.objects
WHERE object_id = OBJECT_ID(N'[apps].[RegistrationsSummaries]') AND type in (N'U'))
BEGIN
...
IF NOT EXISTS (SELECT 1
FROM SYS.COLUMNS
WHERE [Name] = N'NEW_FLAG'
    AND object_id = OBJECT_ID(N'[apps].[SubmissionsSummaries]'))
BEGIN
    ALTER TABLE [apps].[SubmissionsSummaries] ADD [NEW_FLAG] BIT NULL
END;
