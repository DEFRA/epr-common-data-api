IF EXISTS (SELECT 1 FROM sys.procedures WHERE object_id = OBJECT_ID(N'[dbo].[sp_FetchOrganisationRegistrationSubmissionDetails_resub_with_cutoff_date]'))
DROP PROCEDURE [dbo].[sp_FetchOrganisationRegistrationSubmissionDetails_resub_with_cutoff_date];
GO

CREATE PROC [dbo].[sp_FetchOrganisationRegistrationSubmissionDetails_resub_with_cutoff_date] @SubmissionId [nvarchar](36),@LateFeeCutOffDay [int],@LateFeeCutoffMonth [int] AS
...
BEGIN
	...
	SET @LateFeeCutoffDate = DATEFROMPARTS(CONVERT( int, SUBSTRING(
                                ...
                                4
                            )),@LateFeeCutoffMonth, @LateFeeCutOffDay);
		...
		,ComplianceSchemeMembersCTE as (
			select csm.*
				   ...
				   ,CASE WHEN ss.RegistrationDecisionDate IS NULL THEN 0
					...
					END as IsNewJoiner
			from dbo.fn_ComplianceSchemeMembers_resub(@OrganisationUUIDForSubmission,
													  @SubmissionPeriod,
													  @ComplianceSchemeId,
													  @LateFeeCutoffDay,
													  @LateFeeCutoffMonth) csm
