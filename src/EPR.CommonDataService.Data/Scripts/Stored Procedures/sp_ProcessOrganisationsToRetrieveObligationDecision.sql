...
CREATE PROC [dbo].[sp_ProcessOrganisationsToRetrieveObligationDecision] @DpOrgIds [NVARCHAR](MAX),@SubOrgIds [NVARCHAR](MAX) AS
BEGIN
    SET NOCOUNT ON;
    ...
    WITH Split_Dp_OrgIds AS (
        SELECT value AS OrganisationId FROM STRING_SPLIT(@DpOrgIds, @Delimiter)
    )
    INSERT INTO #DpOrganisationIdTable (OrganisationId)
    SELECT OrganisationId FROM Split_Dp_OrgIds;

    WITH Split_Sub_OrgIds AS (
        SELECT value AS OrganisationId FROM STRING_SPLIT(@SubOrgIds, @Delimiter)
    )
    INSERT INTO #SubOrganisationIdTable (OrganisationId)
    SELECT OrganisationId FROM Split_Sub_OrgIds;
    ...
    FROM (
        SELECT
            CAST(
                CASE 
                    WHEN CD.subsidiary_id IS NULL THEN CAST(CD.organisation_id AS NVARCHAR(50))
                    ELSE CD.subsidiary_id 
                END AS NVARCHAR(50)
            ) AS OrganisationId,
            COALESCE(CFM.ComplianceSchemeId, O.ExternalId) AS SubmitterId,
            CD.leaver_code AS LeaverCode,
            CASE 
                WHEN CD.subsidiary_id IS NULL THEN 'DirectRegistrant'
                ELSE 'Subsidiary'
            END AS OrganisationType,
            CASE 
                WHEN CD.subsidiary_id IS NULL THEN NULL 
                ELSE CAST(CD.organisation_id AS NVARCHAR(50))
            END AS ParentOrganisationId,
            ...
            ROW_NUMBER() OVER (
                PARTITION BY 
                    CASE 
                        ...
                        ELSE CD.subsidiary_id 
                    END,
                    COALESCE(CFM.ComplianceSchemeId, O.ExternalId)
    ...
    SELECT LR.*,
        CASE
            WHEN LR.LeaverCode IS NULL THEN NULL
    ...
    SELECT
        ...
        CASE
            -- Explicit invalid flag on record itself
            ...
            WHEN OFS.TotalCount > 1 AND OFS.ObligatedCount = 1 THEN
                CASE 
                    WHEN OO.IsObligated = 1 THEN 1
                    ELSE 0 
                END
            ...
            WHEN OFS.TotalCount > 1 AND OFS.NotObligatedCount > 1 AND OFS.NullCount = 1 THEN 
                CASE 
                    WHEN OO.IsObligated IS NULL THEN 1
                    ELSE 0
                END
            ...
            WHEN OFS.TotalCount = 1 THEN
                CASE 
                    WHEN OO.IsObligated = 1 OR OO.IsObligated IS NULL THEN 1
                    ELSE 0
                END
            ELSE 0
        END AS IsObligatedAfterDecision,
        CASE 
            WHEN 
                (
                    OO.IsObligated = -1 OR
                    (OFS.TotalCount > 1 AND OFS.InvalidCount = OFS.TotalCount)
                ) THEN 'Invalid Leaver Code'
            WHEN 
                (
                    (OFS.TotalCount > 1 AND OFS.NullCount = OFS.TotalCount) OR
                    (OFS.TotalCount > 1 AND OFS.ObligatedCount > 1) OR
                    (OFS.TotalCount > 1 AND OFS.NotObligatedCount = 1 AND OFS.NullCount > 1)
                ) THEN 'Cannot Determine | Error'
            WHEN 
                (
                    (OFS.TotalCount > 1 AND OFS.ObligatedCount = 1 AND OO.IsObligated = 1)
            ...
            ELSE 'Not Obligated'
        END AS IsObligatedAfterDecisionDescription
    ...
    UPDATE OOAI
    SET 
        OOAI.IsObligatedAfterInheritance = COALESCE(P.DpDecision, OOAI.IsObligatedAfterInheritance),
    ...
    LEFT JOIN (
        -- Filter parent DirectRegistrant orgs with Decision <> 1
        SELECT 
            OrganisationId AS DpOrganisationId,
        ...
        ON OOAI.ParentOrganisationId = P.DpOrganisationId
       AND OOAI.SubmitterId = P.DpSubmitterId
