IF EXISTS (SELECT 1 FROM sys.procedures WHERE object_id = OBJECT_ID(N'[dbo].[sp_FetchOrganisationRegistrationSubmissionDetails_resub_nocso]'))
DROP PROCEDURE [dbo].[sp_FetchOrganisationRegistrationSubmissionDetails_resub_nocso];
GO

CREATE PROC [dbo].[sp_FetchOrganisationRegistrationSubmissionDetails_resub_nocso] @SubmissionId [nvarchar](36) AS
...
BEGIN
    ...
    SET @LateFeeCutoffDate =
        DATEFROMPARTS(
        CAST(RIGHT(RTRIM(@SubmissionPeriod), 4) AS INT),
        4,
        1
        );
